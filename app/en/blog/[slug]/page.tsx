...
export async function generateMetadata({ params }: BlogPostEnPageProps): Promise<Metadata> {
...
export default async function BlogPostEnPage({ params }: BlogPostEnPageProps) {
  const { slug } = await params;
  const post = getPostBySlug(slug, "en");
      ...
      <div className="container-shell">
        <div className="panel-card p-7 md:p-10">
          ...
          <p className="mt-4 text-xs font-semibold uppercase tracking-[0.14em] text-[color:var(--color-secondary)]">
            {formatter.format(new Date(post.publishedAt))} · {post.readingMinutes} min read
          </p>
          ...
          />
          <div className="mt-9">
            <Link href="/en/blog" className="btn-base btn-primary px-5 py-3">
              Back to blog
            </Link>
          </div>
        </div>
